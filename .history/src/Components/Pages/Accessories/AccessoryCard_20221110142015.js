...
const AccessoryCard = (props) => {
    ...
    useEffect(() => {
        getDocs(collection(db, "inventorystock/")).then((res) => {
            ...
            res.forEach((doc) => {
                getDocs(collection(db, "inventorystock/" , doc.id, 'summer')).then((response) => {
                    let coloursList =[];
                    ...
                    console.log('summer: ', prodList)

                },[]);


            });
            console.log('summer:test ', prodList)
            setStock(prodList)
        });
    }, []);
    return(
        <div key={props.id}>
             <Grid   marginLeft={10} marginRight={3}  >
            <Grid container spacing={2}  marginTop={2}  marginLeft={10} item xs={20} md={20} lg={12} padding={1}>
                
                    {prodList.map((id,index)=>{
                        console.log("===========",id);
                        return(
                            <Card sx={{ maxWidth: 220}} padding={10} className='productCard'  key={id}>
                            <CardMedia
                                component="img"
                                height="194"
                                src={id.image} 
